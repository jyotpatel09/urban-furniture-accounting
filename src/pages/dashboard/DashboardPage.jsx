...
import { 
  ...
  PlusCircle, ShoppingBag, ShoppingCart, FileText, CreditCard, UserPlus,
...
export const DashboardPage = () => {
  ...
  const [newSoProduct, setNewSoProduct] = useState('');
  const [newSoQty, setNewSoQty] = useState(2);

  const [newPoVendor, setNewPoVendor] = useState('');
  const [newPoProduct, setNewPoProduct] = useState('');
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((e) => console.error('Error hydrating dashboard:', e))
      .finally(() => {
        if (isMounted) setLoading(false);
      });
    return () => { isMounted = false; };
  }, [fetchDashboardSummary, fetchSalesOrders, fetchPurchaseOrders, fetchContacts, fetchProducts]);

  useEffect(() => {
    if (contacts.length > 0 && !newSoCustomer) {
      setNewSoCustomer(contacts[0].name);
    }
    const vend = contacts.find(c => c.type === 'Vendor' || c.type === 'VENDOR');
    if (vend && !newPoVendor) {
      setNewPoVendor(vend.name);
    }
    if (products.length > 0 && !newSoProduct) {
      setNewSoProduct(products[0].id);
      setNewPoProduct(products[0].id);
    }
  }, [contacts, products, newSoCustomer, newPoVendor, newSoProduct]);
  ...
  const recentTransactions = salesOrders.slice(0, 5).map(s => ({
    date: s.orderDate || s.date || 'Today',
    ref: s.number || s.id,
    contact: s.customer?.name || s.customer || 'N/A',
    type: 'Sales',
    amount: Number(s.totalAmount || s.total || 0),
  ...
  const handleCreateSO = async (e) => {
    ...
    if (!prod) return;
    const subtotal = Number(prod.salesPrice) * Number(newSoQty);
    const tax = Math.round(subtotal * (Number(prod.taxRate) / 100));

    await addSalesOrder({
      customerId: contacts.find(c => c.name === newSoCustomer)?.id || contacts[0]?.id,
      items: [{ productId: prod.id, quantity: Number(newSoQty), unitPrice: Number(prod.salesPrice), taxRate: Number(prod.taxRate) }]
    });
    setActiveModal(null);
  ...
  const handleCreatePO = async (e) => {
    ...
    if (!prod) return;
    const subtotal = Number(prod.purchasePrice) * Number(newPoQty);
    const tax = Math.round(subtotal * (Number(prod.taxRate) / 100));

    await addPurchaseOrder({
      vendorId: contacts.find(c => c.name === newPoVendor)?.id || contacts[0]?.id,
      items: [{ productId: prod.id, quantity: Number(newPoQty), unitPrice: Number(prod.purchasePrice), taxRate: Number(prod.taxRate) }]
    });
    setActiveModal(null);
  ...
  const handleCreateContact = async (e) => {
    ...
    await addContact({
      name: newContactName,
      type: newContactType.toUpperCase() === 'CUSTOMER' ? 'CUSTOMER' : newContactType.toUpperCase() === 'VENDOR' ? 'VENDOR' : 'BOTH',
  ...
  const handleCreateProduct = async (e) => {
    ...
    await addProduct({
      ...
      type: 'GOODS',
      taxRate: 18,
      sku: `FURN-${newProdName.slice(0, 3).toUpperCase()}-${Math.floor(1000 + Math.random() * 9000)}`,
  ...
  const totalSalesVal = dashboardSummary ? `₹${Number(dashboardSummary.totalSales || 0).toLocaleString('en-IN')}` : '₹12,45,000';
  const totalPurchasesVal = dashboardSummary ? `₹${Number(dashboardSummary.totalPurchases || 0).toLocaleString('en-IN')}` : '₹7,82,000';
  const receivablesVal = dashboardSummary ? `₹${Number(dashboardSummary.receivables || 0).toLocaleString('en-IN')}` : '₹3,45,000';
  const payablesVal = dashboardSummary ? `₹${Number(dashboardSummary.payables || 0).toLocaleString('en-IN')}` : '₹2,18,000';
  const netProfitVal = dashboardSummary ? `₹${Number(dashboardSummary.netProfit || 0).toLocaleString('en-IN')}` : '₹2,63,000';
  const cashAndBankVal = dashboardSummary ? `₹${Number(dashboardSummary.cashAndBank || 0).toLocaleString('en-IN')}` : '₹5,72,000';
  ...
  return (
    <div className="space-y-6 min-w-0">
      ...
      <div className="bg-white rounded-2xl border border-slate-200/90 p-5 sm:p-6 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4 min-w-0">
        <div className="min-w-0">
          ...
          <p className="text-xs text-slate-500 mt-1">
            Here's your real-time PostgreSQL database accounting overview for <span className="font-semibold text-purple-900">Urban Furniture</span>.
          </p>
        </div>

        {/* Hero Actions (Role-Scoped) */}
        <div className="flex items-center gap-2.5 flex-wrap shrink-0">
          {(user?.role === 'Administrator' || user?.role === 'Admin' || user?.role === 'ADMIN' || user?.role === 'Sales & Purchase User' || user?.role === 'SALES_PURCHASE') && (
          ...
          {(user?.role === 'Administrator' || user?.role === 'Admin' || user?.role === 'ADMIN' || user?.role === 'Accountant' || user?.role === 'ACCOUNTANT') && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-6 gap-3.5 sm:gap-4 min-w-0">
        <StatCard
          title="Total Sales"
          value={totalSalesVal}
          change="12.5%"
          ...
          <div className="h-72 w-full flex flex-col items-center justify-between min-w-0">
            <div className="relative w-full h-48 flex items-center justify-center">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                ...
                <span className="text-base font-extrabold text-slate-900">{totalSalesVal}</span>
              </div>
            </div>
      ...
      {/* 5. Accounting Health & Insight Banner */}
      <Card title="Accounting Health & Financial Position" subtitle="Real-time liquidity and budget utilization indicators">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
          <div className="p-3 bg-purple-50/50 rounded-xl border border-purple-100">
            ...
            <span className="text-lg font-bold text-purple-900 block mt-0.5">{receivablesVal}</span>
          ...
          <div className="p-3 bg-amber-50/50 rounded-xl border border-amber-100">
            ...
            <span className="text-lg font-bold text-amber-900 block mt-0.5">{payablesVal}</span>
          ...
          <div className="p-3 bg-emerald-50/50 rounded-xl border border-emerald-100">
            ...
            <span className="text-lg font-bold text-emerald-900 block mt-0.5">{netProfitVal}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 min-w-0">
        {/* Quick Actions Panel (Role Scoped) */}
            ...
            {recentActivity.map((act, idx) => {
              ...
              return (
                <div key={idx} className="p-3 bg-slate-50/80 rounded-xl border border-slate-100 flex items-start gap-3 text-xs">
                  <div className={`p-2 rounded-lg bg-white border border-slate-200 ${act.color || 'text-purple-600'} shrink-0 mt-0.5`}>
                    <Icon className="w-4 h-4" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <span className="font-bold text-slate-900 truncate">{act.title || act.subtitle}</span>
                      <span className="text-[10px] text-slate-400 font-mono shrink-0">{act.time || act.date || 'Just now'}</span>
                    </div>
                    <p className="text-slate-600 mt-0.5 truncate">{act.desc || act.amount}</p>
                  </div>
                </div>
              );
            })}
      ...
      <Modal isOpen={activeModal === 'so'} onClose={() => setActiveModal(null)} title="+ Create New Sales Order">
        <form onSubmit={handleCreateSO} className="space-y-4">
          <Select
            ...
            options={contacts.filter(c => c.type !== 'Vendor' && c.type !== 'VENDOR').map(c => ({ label: c.name, value: c.name }))}
          />
          <Select
            label="Product"
            required
            ...
            options={products.map(p => ({ label: `${p.name} (Sales: ₹${Number(p.salesPrice).toLocaleString('en-IN')})`, value: p.id }))}
          />
          <Input
            label="Quantity"
            type="number"
          ...
          <div className="p-3 bg-purple-50 rounded-xl text-xs text-purple-900 border border-purple-100">
            ...
            <p>Creating this SO will generate a real database sales order & permit 1-click invoice creation.</p>
          </div>
      ...
      <Modal isOpen={activeModal === 'po'} onClose={() => setActiveModal(null)} title="+ Create New Purchase Order">
        <form onSubmit={handleCreatePO} className="space-y-4">
          <Select
            ...
            options={contacts.filter(c => c.type !== 'Customer' && c.type !== 'CUSTOMER').map(c => ({ label: c.name, value: c.name }))}
          />
          <Select
            label="Product"
            required
            ...
            options={products.map(p => ({ label: `${p.name} (Cost: ₹${Number(p.purchasePrice).toLocaleString('en-IN')})`, value: p.id }))}
          />
          <Input
            label="Quantity"
            type="number"
