...
import { ArrowLeft, CheckCircle, AlertTriangle, Layers, BookOpen } from 'lucide-react';

export const JournalEntryDetailPage = () => {
  const { id } = useParams();
  const navigate = useNavigate();

  const je = useStore((state) => state.journalEntries.find((j) => j.id === id)) || useStore((state) => state.journalEntries[0]);

  if (!je) return <div className="p-8 text-center text-gray-500">Journal Entry not found.</div>;

  const isBalanced = je.totalDebit === je.totalCredit;
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        title={`Journal Entry ${je.id}`}
        subtitle={`Journal: ${je.journal} • Reference: ${je.reference}`}
        breadcrumbs={['Dashboard', 'Journal Entries', je.id]}
        actions={
        ...
        <Card className="lg:col-span-2" title="Journal Line Items">
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {je.items?.map((item, idx) => (
                  <tr key={idx}>
                    <td className="py-3.5 px-4 font-mono font-bold text-purple-900">{item.accountCode}</td>
                    <td className="py-3.5 px-4 font-medium text-gray-900">{item.accountName}</td>
          ...
          <div className="border-t border-gray-200 pt-4 mt-4 flex justify-between items-center text-sm font-bold">
            ...
            <div className="flex gap-8">
              <span className="text-purple-900">Dr ₹{je.totalDebit.toLocaleString('en-IN')}</span>
              <span className="text-teal-800">Cr ₹{je.totalCredit.toLocaleString('en-IN')}</span>
            </div>
          </div>
        </Card>
        ...
        <Card title="Voucher Metadata">
          <div className="space-y-4 text-xs">
            <div>
              ...
              <span className="text-sm font-bold text-gray-900">{je.journal}</span>
            </div>
            <div>
            ...
            <div>
              ...
              <span className="font-medium text-gray-800">{je.date}</span>
            </div>
            <div>
